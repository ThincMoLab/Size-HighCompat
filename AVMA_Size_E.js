...
signInAnonymously(auth).catch((e) => console.warn('Anon auth failed', e)); // fire-and-forget is fine here

const db = getFirestore(app);
const storage = getStorage(app); */


/* // Create a unique session ID (available everywhere)
window.sessionId = `${expName}_${(expInfo['participant'] || 'anon')}_${Date.now()}`; */
...
var current_RT_block;
var current_TR_block_old;
var current_TR_block_new;
//////////////////////////////////////
...
num_trials_sound_check = 20;
  ...
  // initiaze one shared clock for all keyboard objects
  sharedKeyClock = new util.Clock();
  ...
  Instr_Exp_Press = new core.Keyboard({psychoJS: psychoJS, clock: new util.Clock(), waitForStart: true});
  ...
  Sound_Check_Press = new core.Keyboard({psychoJS: psychoJS, clock: sharedKeyClock, waitForStart: true});
 ...
 HandShape = new visual.ImageStim({
    win : psychoJS.window,
    name : 'HandShape', units : 'height', 
  ...
  TR_Penalty_Press = new core.Keyboard({psychoJS: psychoJS, clock: sharedKeyClock, waitForStart: true});
  ...
  RT_Press = new core.Keyboard({psychoJS: psychoJS, clock: sharedKeyClock, waitForStart: true});
...
function TR_Old_Pre_BoolLoopBegin(thisScheduler) {
  current_TR_block_old = tr_block_old;
  // set up handler to look after randomisation of conditions etc
...
function TR_Old_Block_PreLoopBegin(thisScheduler) {
  ...
  TR_Old_Block_Pre = new TrialHandler({
    psychoJS: psychoJS,
    nReps: current_TR_block_old, method: TrialHandler.Method.SEQUENTIAL,
    extraInfo: expInfo, originPath: undefined,
    trialList: undefined,
...
function RT_BlockLoopBegin(thisScheduler) {
  ...
  RT_Block = new TrialHandler({
    psychoJS: psychoJS,
    nReps: current_RT_block, method: TrialHandler.Method.SEQUENTIAL,
    extraInfo: expInfo, originPath: undefined,
    trialList: undefined,
...
function TR_New_BoolLoopBegin(thisScheduler) {
  current_TR_block_new = tr_block_new_swap;
  // set up handler to look after randomisation of conditions etc
...
function Init_StimRoutineEnd(trials) {
  return function () {
    ...
    if (session < 3) {
      instr_end_exp_text = `Great job! You have completed today's session.
...
This is a 3-day study and it will take about 45 minutes today. 
...
Press (space) to continue.`
  ;
      ...
      instr_exp_text = `Welcome back! Today's session will take about 35 minutes.

Take a break between blocks whenever you need. Please DO NOT turn off your web browser.
          
Whenever you are ready, press (space) to start.`
          ;
    } else if (session === 3) {
      instr_exp_text = `Today's session will be a bit longer than before. It will take about 1 hour and 25 minutes.
...
Whenever you are ready, press (space) to start.`
      ...
      instr_exp_text = `Thank you coming back for this follow-up study!.
...
Whenever you are ready, press (H, U, I, or L) to start.`
  ;
          ...
          instr_rt_pre_text = `In today's session, We will continue to practice the symbol-key map you did yesterday.
  
Remember, your job is to press the corresponding key as quickly and accurately as you can.

There will be ${rt_pre_block} blocks.

Whenever you are ready, press (H, U, I, or L) to start.`
  ;
  } else if (session === 3) {
          instr_rt_pre_text = `Today, we first continue to practice the symbol-key maps for ${rt_pre_block} blocks.
...
Press (H, U, I, or L) to start.`
 ...
 /*          instr_cr_new_text = `Good Job! You are now ready for the tasks!
...
function Instr_ExpRoutineBegin(trials) {
  return function () {
    ...
    Instr_ExpComponents.push(Instr_Exp_Press);
...
function Instr_ExpRoutineEachFrame(trials) {
  return function () {
    ...
    if (Instr_Exp_Press.status === PsychoJS.Status.STARTED) {
      let theseKeys = Instr_Exp_Press.getKeys({keyList: ['space'], waitRelease: false});
      _allKeys = _allKeys.concat(theseKeys);
      if (_allKeys.length > 0) {
        Instr_Exp_Press.keys = _allKeys[0].name;  // just the first key pressed
        Instr_Exp_Press.rt = _allKeys[0].rt;
...
function Instr_Sound_CheckRoutineBegin(trials) {
  return function () {
    ...
    frameN = -1;
    // update component parameters for each repeat
    Instr_Exp_Text.setText(instr_sound_check_text);
    Instr_Exp_Press.keys = undefined;
    Instr_Exp_Press.rt = undefined;
    _allKeys = [];
    trial_count = 0;
    ...
    Instr_Sound_CheckComponents.push(Instr_Exp_Text);
    Instr_Sound_CheckComponents.push(Instr_Exp_Press);
    Instr_Sound_CheckComponents.push(Feedback_Coin);
...
function Instr_Sound_CheckRoutineEachFrame(trials) {
  return function () {
    ...
    if (Instr_Exp_Press.status === PsychoJS.Status.STARTED) {
      let theseKeys = Instr_Exp_Press.getKeys({keyList: ['f', 'j'], waitRelease: false});
      _allKeys = _allKeys.concat(theseKeys);
      if (_allKeys.length > 0) {
        Instr_Exp_Press.keys = _allKeys[0].name;  // just the last key pressed
        Instr_Exp_Press.rt = _allKeys[0].rt;
      }
    }
    
    
    // start/stop Feedback_Coin
    if (Instr_Exp_Press.keys === "f" && Feedback_Coin.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      Feedback_Coin.tStart = t;  // (not accounting for frame time here)
      Feedback_Coin.frameNStart = frameN;  // exact frame index
      temp_t = Feedback_Coin.tStart


      psychoJS.window.callOnFlip(function(){ Feedback_Coin.play(); });  // screen flip
      Feedback_Coin.status = PsychoJS.Status.STARTED;
    }
    if (t >= (Feedback_Coin.getDuration() + Feedback_Coin.tStart)     && Feedback_Coin.status === PsychoJS.Status.STARTED) {
      Feedback_Coin.stop();  // stop the sound (if longer than duration)
      Feedback_Coin.status = PsychoJS.Status.FINISHED;
    }
    // start/stop sound_check_buzz
    if (Instr_Exp_Press.keys === "j" && sound_check_buzz.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
    ...
    if (Feedback_Coin.status === PsychoJS.Status.FINISHED || sound_check_buzz.status === PsychoJS.Status.FINISHED) {
      continueRoutine = false;
    }
...
function Instr_Sound_CheckRoutineEnd(trials) {
  return function () {
    ...
    Instr_Exp_Press.stop();
    if ((Instr_Exp_Press.keys === "space")) {
        trials.finished = true;
    }
    
    //Feedback_Coin.stop();  // ensure sound has stopped at end of routine
...
function Sound_Check_Creat_StimSeqRoutineBegin(trials) {
  return function () {
    ...
    Instr_Exp_Text.setText(instr_sound_check_text2);
    Instr_Exp_Press.keys = undefined;
    Instr_Exp_Press.rt = undefined;
    _allKeys = [];
    ...
    Sound_Check_Creat_StimSeqComponents.push(Instr_Exp_Text);
    Sound_Check_Creat_StimSeqComponents.push(Instr_Exp_Press);
...
function Sound_Check_Enter_TrialRoutineBegin(trials) {
  return function () {
    ...
    Instr_Exp_Text.setText(instr_sound_select_text);
    // keep track of which components have finished
    ...
    Sound_Check_Enter_TrialComponents.push(Instr_Exp_Text);
...
function Sound_Check_Enter_TrialRoutineEachFrame(trials) {
  return function () {
    ...
    if (Sound_Check_Press.status === PsychoJS.Status.STARTED) {
      ...
      _allKeys = _allKeys.concat(theseKeys);
      if (_allKeys.length > 0) {
        Sound_Check_Press.keys = _allKeys[0].name;  // just the first key pressed
        Sound_Check_Press.rt = _allKeys[0].rt;
        // a response ends the routine
        continueRoutine = false;
      }
    }
    ...
    /* // *Instr_Exp_Text* updates
    if (t >= 2 && Instr_Exp_Text.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      Instr_Exp_Text.tStart = t;  // (not accounting for frame time here)
      Instr_Exp_Text.frameNStart = frameN;  // exact frame index
...
function Sound_Quit_TextRoutineBegin(trials) {
  return function () {
    ...
    frameN = -1;
    // update component parameters for each repeat
    Instr_Exp_Press.setText(instr_sound_quit_text);
    // keep track of which components have finished
    Sound_Quit_TextComponents = [];
    Sound_Quit_TextComponents.push(Instr_Exp_Press);
...
function Sound_Quit_TextRoutineEachFrame(trials) {
  return function () {
    ...
    if (t >= 0.0 && Instr_Exp_Press.status === PsychoJS.Status.NOT_STARTED) {
      ...
      Instr_Exp_Press.setAutoDraw(true);
    }
...
function Sound_Exit_TextRoutineBegin(trials) {
  return function () {
    ...
    frameN = -1;
    // update component parameters for each repeat
    Instr_Exp_Text.setText(instr_sound_exit_text);
    ...
    Sound_Exit_TextComponents.push(Instr_Exp_Text);
    Sound_Exit_TextComponents.push(Instr_Exp_Press);
...
function Instr_RT_HandRoutineBegin(trials) {
  return function () {
    ...
    Instr_Exp_Text.setText(instr_rt_text_hand);
    Instr_Exp_Press.keys = undefined;
    Instr_Exp_Press.rt = undefined;
    _allKeys = [];
    // keep track of which components have finished
    Instr_RT_HandComponents = [];
    Instr_RT_HandComponents.push(Instr_Exp_Text);
    Instr_RT_HandComponents.push(Instr_Exp_Press);
...
function Instr_RT_HandRoutineEachFrame(trials) {
  return function () {
    ...
    if (Instr_Exp_Press.status === PsychoJS.Status.STARTED) {
      let theseKeys = Instr_Exp_Press.getKeys({keyList: ['h','u','i','l'], waitRelease: false});
      _allKeys = _allKeys.concat(theseKeys);
      if (_allKeys.length > 0) {
        Instr_Exp_Press.keys = _allKeys[0].name;  // just the first key pressed
        Instr_Exp_Press.rt = _allKeys[0].rt;
...
function Pre_Trial_HandRoutineBegin(trials) {
  return function () {
   ...
   // Pre_Trial_HandComponents.push(Rec_Frame_Hand);
    Pre_Trial_HandComponents.push(HandShape);
...
function Pre_Trial_HandRoutineEachFrame(trials) {
  return function () {
    ...
    // // *Rec_Frame_Hand* updates
    // if (t >= 0.0 && Rec_Frame_Hand.status === PsychoJS.Status.NOT_STARTED) {
    //   // keep track of start time/frame for later
    //   Rec_Frame_Hand.tStart = t;  // (not accounting for frame time here)
    //   Rec_Frame_Hand.frameNStart = frameN;  // exact frame index
      
    //   Rec_Frame_Hand.setAutoDraw(true);
    // }

    /* frameRemains = 0.0 + pre_trial_dur - psychoJS.window.monitorFramePeriod * 0.75;  // most of one frame period left
    if (Rec_Frame_Hand.status === PsychoJS.Status.STARTED && t >= frameRemains) {
      Rec_Frame_Hand.setAutoDraw(false);
    } */
    
    // *HandShape* updates
    if (t >= 0.0 && HandShape.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      HandShape.tStart = t;  // (not accounting for frame time here)
      HandShape.frameNStart = frameN;  // exact frame index
      
      HandShape.setAutoDraw(true);
    }

    frameRemains = 0.0 + pre_trial_dur - psychoJS.window.monitorFramePeriod * 0.75;  // most of one frame period left
    if (HandShape.status === PsychoJS.Status.STARTED && t >= frameRemains) {
      HandShape.setAutoDraw(false);
    }
    // check for quit (typically the Esc key)
    if (psychoJS.experiment.experimentEnded || psychoJS.eventManager.getKeys({keyList:['escape']}).length > 0) {
      return quitPsychoJS('The [Escape] key was pressed. Goodbye!', false);
...
function Pre_Trial_HandRoutineEnd(trials) {
  return function () {
    ...
    psychoJS.experiment.addData("pre_rt", pre_rt);
    psychoJS.experiment.addData("set_size", set_size);
...
function RT_Enter_Trial_HandRoutineBegin(trials) {
  return function () {
   ...
   // RT_Enter_Trial_HandComponents.push(Rec_Frame);
    RT_Enter_Trial_HandComponents.push(RT_Press);
    RT_Enter_Trial_HandComponents.push(HandShape);
...
function RT_Enter_Trial_HandRoutineEachFrame(trials) {
  return function () {
    ...
    // if (t >= 0.0 && Rec_Frame_Hand.status === PsychoJS.Status.NOT_STARTED) {
    //   // keep track of start time/frame for later
    //   Rec_Frame.tStart = t;  // (not accounting for frame time here)
    //   Rec_Frame.frameNStart = frameN;  // exact frame index
      
    //   Rec_Frame.setAutoDraw(true);
    // }
    ...
    if (RT_Press.status === PsychoJS.Status.STARTED) {
      ...
      _allKeys = _allKeys.concat(theseKeys);
      if (_allKeys.length > 0) {
        RT_Press.keys = _allKeys[0].name;  // just the first key pressed
        RT_Press.rt = _allKeys[0].rt;
        // was this correct?
...
function RT_Feedback_HandRoutineBegin(trials) {
  return function () {
    ...
    Feedback_Coin.setVolume(sound_vol);
    //Rec_Frame.setOpacity(rec_frame_opacity);
    //Rec_Frame.setLineColor(new util.Color(rec_frame_color));
    //Rec_Frame.setLineWidth(rec_wd);
    HandShape.setImage(hand_shape);
    ...
    RT_Feedback_HandComponents.push(Feedback_Coin);
    //RT_Feedback_HandComponents.push(Rec_Frame);
    RT_Feedback_HandComponents.push(HandShape);
    RT_Feedback_HandComponents.push(RT_Stim_Hand);
...
function RT_Feedback_HandRoutineEachFrame(trials) {
  return function () {
    ...
    // update/draw components on each frame
    // start/stop Feedback_Coin
    if (t >= 0.0 && Feedback_Coin.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      Feedback_Coin.tStart = t;  // (not accounting for frame time here)
      Feedback_Coin.frameNStart = frameN;  // exact frame index
    ...
    if (t >= frameRemains && Feedback_Coin.status === PsychoJS.Status.STARTED) {
    //  Feedback_Coin.stop();  // stop the sound (if longer than duration)
      Feedback_Coin.status = PsychoJS.Status.FINISHED;
    }
    
    // // *Rec_Frame* updates
    // if (t >= 0.0 && Rec_Frame.status === PsychoJS.Status.NOT_STARTED) {
    //   // keep track of start time/frame for later
    //   Rec_Frame.tStart = t;  // (not accounting for frame time here)
    //   Rec_Frame.frameNStart = frameN;  // exact frame index
    ...
    // if (Rec_Frame.status === PsychoJS.Status.STARTED && t >= frameRemains) {
    //   Rec_Frame.setAutoDraw(false);
    // }
    ...
    frameRemains = 0.0 + feedback_dur - psychoJS.window.monitorFramePeriod * 0.75;  // most of one frame period left
    if (RT_Stim_Hand.status === PsychoJS.Status.STARTED && t >= frameRemains) {
      RT_Stim_Hand.setAutoDraw(false);
    }
    // check for quit (typically the Esc key)
    if (psychoJS.experiment.experimentEnded || psychoJS.eventManager.getKeys({keyList:['escape']}).length > 0) {
      return quitPsychoJS('The [Escape] key was pressed. Goodbye!', false);
...
function Instr_TR_HandRoutineBegin(trials) {
  return function () {
    ...
    frameN = -1;
    // update component parameters for each repeat
    Instr_Exp_Text.setText(instr_tr_text_hand);
    Instr_Exp_Press.keys = undefined;
    Instr_Exp_Press.rt = undefined;
    _allKeys = [];
    block_type = "TR";
    ...
    Instr_TR_HandComponents.push(Instr_Exp_Text);
    Instr_TR_HandComponents.push(Instr_Exp_Press);
  ...
  return function () {
    ...
    if (block_type === "TR" && stim_type === "Symb" && remap === 0) {
        if (set_size === 4) {
          ...
          '\n\nOnly 4 symbols and their corresponding keys (' + subset_key + ') will be used' +
          '\n\n\nPress (' + subset_key + ')  to start'));
        } else {
          Instr_Block_Num_Text.setText((('Block ' + block_count) + 
          '\n\nAll 8 symbols and 8 keys will be used' +
          '\n\n\nPress (H, U, I, or L) to start'));
        } 
    } else if (block_type === "RT" && stim_type === "Symb" && pre_rt === 0) {
      if (set_size === 4) {
    ...
    Instr_Block_NumComponents.push(Instr_Exp_Press);
...
function Instr_Block_NumRoutineEachFrame(trials) {
  return function () {
    ...
    // *Instr_Exp_Press* updates
    if (t >= 1 && Instr_Exp_Press.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      Instr_Exp_Press.tStart = t;  // (not accounting for frame time here)
      Instr_Exp_Press.frameNStart = frameN;  // exact frame index
    ...
    if (Instr_Exp_Press.status === PsychoJS.Status.STARTED) {
      let theseKeys = Instr_Exp_Press.getKeys({keyList: key_list_1, waitRelease: false});
      _allKeys = _allKeys.concat(theseKeys);
      if (_allKeys.length > 0) {
        Instr_Exp_Press.keys = _allKeys[0].name;  // just the first key pressed
        Instr_Exp_Press.rt = _allKeys[0].rt;
...
function TR_Enter_Trials_HandRoutineBegin(trials) {
  ...
  //  Rec_Frame.setLineColor(new util.Color(rec_frame_color));
   ...
   // TR_Enter_Trials_HandComponents.push(Rec_Frame);
    TR_Enter_Trials_HandComponents.push(HandShape);
    TR_Enter_Trials_HandComponents.push(RT_Stim_Hand);
    TR_Enter_Trials_HandComponents.push(RT_Press);
...
function TR_Enter_Trials_HandRoutineEachFrame(trials) {
  return function () {
    ...
    // *RT_Stim_Hand* updates
    if (t >= stim_start_time && RT_Stim_Hand.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      RT_Stim_Hand.tStart = t;  // (not accounting for frame time here)
      RT_Stim_Hand.frameNStart = frameN;  // exact frame index
    ...
    if (RT_Press.status === PsychoJS.Status.STARTED && t >= frameRemains) {
      RT_Press.status = PsychoJS.Status.FINISHED;
  }
...
After 2 second, press  (${subset_key}) to continue.`
    ...
    if ((RT_Press.keys !== undefined)) {
        ...
        if ((RT_Press.rt < ((time_limit - 0.2) - 0.3))) {
            tr_press_tooearly = 1;
            tr_penalty = 1;
        } else {
            tr_feedback = 1;
            tr_timing_good = (tr_timing_good + 1);
            if ((RT_Press.rt < ((time_limit - 0.2) - timing_tol_early))) {
                tr_press_early = 1;
            } else {
                if ((RT_Press.rt > ((time_limit - 0.2) + timing_tol_late))) {
                    tr_press_late = 1;
                }
            }
        }
...
function TR_Feedback_HandRoutineBegin(trials) {
  return function () {
   ...
   // Rec_Frame.setLineWidth(rec_wd);
    ...
    TR_Feedback_HandComponents.push(Feedback_Coin);
   // TR_Feedback_HandComponents.push(Rec_Frame);
    TR_Feedback_HandComponents.push(TR_Feedback_Text);
    TR_Feedback_HandComponents.push(HandShape);
    TR_Feedback_HandComponents.push(RT_Stim_Hand);
...
function TR_Feedback_HandRoutineEachFrame(trials) {
  return function () {
    ...
    // update/draw components on each frame
    // start/stop Feedback_Coin
    if (t >= 0 && Feedback_Coin.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      Feedback_Coin.tStart = t;  // (not accounting for frame time here)
      Feedback_Coin.frameNStart = frameN;  // exact frame index
...
function TR_PenaltyRoutineBegin(trials) {
  ...
  //  TR_PenaltyComponents.push(Rec_Frame);
...
function TR_PenaltyRoutineEachFrame(trials) {
  return function () {
    ...
    if (TR_Penalty_Press.status === PsychoJS.Status.STARTED) {
      ...
      _allKeys = _allKeys.concat(theseKeys);
      if (_allKeys.length > 0) {
        TR_Penalty_Press.keys = _allKeys[0].name;  // just the first key pressed
        TR_Penalty_Press.rt = _allKeys[0].rt;
        // a response ends the routine
        continueRoutine = false;
      }
    }
...
function CountDownRoutineBegin(trials) {
  return function () {
    ...
    CountDownComponents.push(Feedback_Coin);
...
function Instr_CR_OldRoutineBegin(trials) {
  return function () {
    ...
    frameN = -1;
    // update component parameters for each repeat
    Instr_Exp_Text.setText(instr_cr_old_text);
    Instr_Exp_Press.keys = undefined;
    Instr_Exp_Press.rt = undefined;
    _allKeys = [];
    block_type = "CR";
    ...
    Instr_CR_OldComponents.push(Instr_Exp_Text);
    Instr_CR_OldComponents.push(Instr_Exp_Press);
...
function Pre_TrialRoutineBegin(trials) {
  return function () {
    ...
    // keep track of which components have finished
    Pre_TrialComponents = [];
    Pre_TrialComponents.push(Rec_Frame);
...
function RT_Enter_TrialRoutineBegin(trials) {
  return function () {
    ...
    RT_Enter_TrialComponents.push(Rec_Frame);
    RT_Enter_TrialComponents.push(Stim_Image);
...
function RT_FeedbackRoutineBegin(trials) {
  return function () {
    ...
    Rec_Frame.setLineColor(new util.Color(rec_frame_color));
    Rec_Frame.setLineWidth(rec_wd);
    Stim_Image.setImage(feedback_image);
    //Letter.setColor(new util.Color(feedback_col));
    //Letter.setText(symb_item);
    // keep track of which components have finished
    RT_FeedbackComponents = [];
    RT_FeedbackComponents.push(Feedback_Coin);
    RT_FeedbackComponents.push(Rec_Frame);
    RT_FeedbackComponents.push(Stim_Image);
...
function Instr_TR_Old_PreRoutineBegin(trials) {
  return function () {
    ...
    frameN = -1;
    // update component parameters for each repeat
    Instr_Exp_Text.setText(instr_tr_old_pre_text);
    Instr_Exp_Press.keys = undefined;
    Instr_Exp_Press.rt = undefined;
    _allKeys = [];
    block_type = "TR";
    ...
    Instr_TR_Old_PreComponents.push(Instr_Exp_Text);
    Instr_TR_Old_PreComponents.push(Instr_Exp_Press);
...
function TR_Enter_TrialsRoutineBegin(trials) {
  return function () {
    ...
    TR_Enter_TrialsComponents.push(Rec_Frame);
    TR_Enter_TrialsComponents.push(Stim_Image);
    //TR_Enter_TrialsComponents.push(Letter);
    TR_Enter_TrialsComponents.push(RT_Press);
...
function TR_Enter_TrialsRoutineEachFrame(trials) {
  return function () {
    ...
    // *Stim_Image* updates
    if (t >= stim_start_time && Stim_Image.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      Stim_Image.tStart = t;  // (not accounting for frame time here)
      Stim_Image.frameNStart = frameN;  // exact frame index
...
function TR_FeedbackRoutineBegin(trials) {
  return function () {
    ...
    TR_FeedbackComponents.push(Feedback_Coin);
    TR_FeedbackComponents.push(Rec_Frame);
    TR_FeedbackComponents.push(TR_Feedback_Text);
    TR_FeedbackComponents.push(Stim_Image);
...
function Instr_RT_PreRoutineBegin(trials) {
  return function () {
    ...
    frameN = -1;
    // update component parameters for each repeat
    Instr_Exp_Text.setText(instr_rt_pre_text);
    Instr_Exp_Press.keys = undefined;
    Instr_Exp_Press.rt = undefined;
    _allKeys = [];
    block_type = "RT";
    ...
    Instr_RT_PreComponents.push(Instr_Exp_Text);
    Instr_RT_PreComponents.push(Instr_Exp_Press);
...
function Instr_TR_Old_PostRoutineBegin(trials) {
  return function () {
    ...
    frameN = -1;
    // update component parameters for each repeat
    Instr_Exp_Text.setText(instr_tr_old_post_text);
    Instr_Exp_Press.keys = undefined;
    Instr_Exp_Press.rt = undefined;
    _allKeys = [];
    block_type = "TR";
    ...
    Instr_TR_Old_PostComponents.push(Instr_Exp_Text);
    Instr_TR_Old_PostComponents.push(Instr_Exp_Press);
...
function Instr_CR_NewRoutineBegin(trials) {
  return function () {
    ...
    frameN = -1;
    // update component parameters for each repeat
    Instr_Exp_Text.setText(instr_cr_new_text);
    Instr_Exp_Press.keys = undefined;
    Instr_Exp_Press.rt = undefined;
    _allKeys = [];
    block_type = "CR";
    ...
    Instr_CR_NewComponents.push(Instr_Exp_Text);
    Instr_CR_NewComponents.push(Instr_Exp_Press);
...
function RT_Enter_Trial_StopRoutineBegin(trials) {
  return function () {
    ...
    RT_Enter_Trial_StopComponents.push(Rec_Frame);
    RT_Enter_Trial_StopComponents.push(Stim_Image);
    //RT_Enter_Trial_StopComponents.push(Letter);
    RT_Enter_Trial_StopComponents.push(RT_Press);
...
function Instr_TR_NewRoutineBegin(trials) {
  return function () {
    ...
    frameN = -1;
    // update component parameters for each repeat
    Instr_Exp_Text.setText(instr_tr_new_text);
    Instr_Exp_Press.keys = undefined;
    Instr_Exp_Press.rt = undefined;
    _allKeys = [];
    block_type = "TR";
    ...
    Instr_TR_NewComponents.push(Instr_Exp_Text);
    Instr_TR_NewComponents.push(Instr_Exp_Press);
...
function Instr_TR_NewRoutineEachFrame(trials) {
  return function () {
    ...
    if (Instr_Exp_Press.status === PsychoJS.Status.STARTED) {
      let theseKeys = Instr_Exp_Press.getKeys({keyList: key_list, waitRelease: false});
      _allKeys = _allKeys.concat(theseKeys);
      if (_allKeys.length > 0) {
        Instr_Exp_Press.keys = _allKeys[0].name;  // just the first key pressed
        Instr_Exp_Press.rt = _allKeys[0].rt;
...
function TR_Enter_Trials_StopRoutineBegin(trials) {
  return function () {
    ...
    TR_Enter_Trials_StopComponents.push(Rec_Frame);
    TR_Enter_Trials_StopComponents.push(Stim_Image);
    //TR_Enter_Trials_StopComponents.push(Letter);
    TR_Enter_Trials_StopComponents.push(RT_Press);
...
function Instr_End_ExpRoutineBegin(trials) {
  return function () {
    ...
    frameN = -1;
    // update component parameters for each repeat
    Instr_Exp_Text.setText(instr_end_exp_text);
    Instr_Exp_Press.keys = undefined;
    Instr_Exp_Press.rt = undefined;
    _allKeys = [];
    // keep track of which components have finished
    Instr_End_ExpComponents = [];
    Instr_End_ExpComponents.push(Instr_Exp_Text);
    Instr_End_ExpComponents.push(Instr_Exp_Press);
...
/* // Replace your quitPsychoJS function with this:
async function quitPsychoJS(message, isCompleted) {
    ...
    // Get all trial data from PsychoJS
    const allTrials = psychoJS.experiment._trialsData;
    
    // Add participant info to EACH trial (for CSV rows)
    const enrichedTrials = allTrials.map(trial => ({
        participant: expInfo['participant'],
        session: expInfo['session'],
        date: expInfo['date'],
        expName: expName,
        browser: expInfo['browser'],
        frameRate: expInfo['frameRate'],
        OS: expInfo['OS'],
        ...trial  // Spread all trial-specific data
    }));
    
    // Convert to CSV format
    const csv = convertTrialsToCSV(enrichedTrials);
    
    // Save to Firebase
    try {
        await addDoc(collection(db, "experiments"), {
            // Document-level info (for Firebase queries):
            participant: expInfo['participant'],
            session: expInfo['session'],
            date: expInfo['date'],
            expName: expName,
            browser: expInfo['browser'],
            browserInfo: expInfo['browserInfo'],
            frameRate: expInfo['frameRate'],
            OS: expInfo['OS'],
            psychopyVersion: expInfo['psychopyVersion'],
            
            // Data in both formats:
            csvData: csv,                    // CSV as string
            trialsData: enrichedTrials,      // JSON as array
        ...
        alert("Data saved successfully! You may now close this window.");
        console.log("✅ Data saved to Firebase successfully");
        
    } catch (error) {
        console.error("❌ Error saving to Firebase:", error);
        alert("Cloud save failed. Downloading backup file to your computer...");
        
        // Fallback: download to participant's computer
        const filename = `P${expInfo['participant']}_S${expInfo['session']}_${expInfo['date']}.csv`;
...
// Helper function: Convert trial array to CSV string
function convertTrialsToCSV(trials) {
    if (!trials || trials.length === 0) {
        return '';
    }
    
    // Get all unique keys from all trials (in case some trials have different fields)
    ...
    // Create data rows
    const dataRows = trials.map(trial => {
...
// Helper function: Fallback download to participant's computer
