...
for i = 1:size(positions, 1)
    ...
    img(y_top_start:y_top_end, x1:x2, 1:3) = 200;
    img(y_top_start:y_top_end, x1:x2, 4) = 200;
    
    % Bottom border
    y_bot_start = max(y2 - line_width + 1, y1);
    y_bot_end = y2;
    img(y_bot_start:y_bot_end, x1:x2, 1:3) = 200;
    img(y_bot_start:y_bot_end, x1:x2, 4) = 200;
    
    % Left border
    x_left_start = x1;
    x_left_end = min(x1 + line_width - 1, x2);
    img(y1:y2, x_left_start:x_left_end, 1:3) = 200;
    img(y1:y2, x_left_start:x_left_end, 4) = 200;
    
    % Right border
    x_right_start = max(x2 - line_width + 1, x1);
    x_right_end = x2;
    img(y1:y2, x_right_start:x_right_end, 1:3) = 200;
    img(y1:y2, x_right_start:x_right_end, 4) = 200;
end
